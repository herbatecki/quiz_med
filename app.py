...
app = Flask(__name__)
app.secret_key = 'supersecretkey'  # Klucz potrzebny do korzystania z sesji
...
\n6) rak płuca może przebiegać ze wzrostem stężenia antygenu rakowo-płodowego (CEA) w surowicy krwi.\n""",
         ...
         Question(
            prompt="""2. Wskaż prawdziwe stwierdzenia:
...
Question(
            prompt="""3. Wskaż prawdziwe stwierdzenia:
...
Question(
            prompt="""4. Wskaż zdania prawdziwe:
...
Question(
            prompt="""5. Wskaż zdania prawdziwe:
...
Question(
            prompt="""6. Wskaż lek, który nie jest lekiem pierwszego rzutu w leczeniu nadciśnienia tętniczego:""",
...
Question(
            prompt="""7. Wskaż diuretyk, który powinien być użyty w pierwszej kolejności w leczeniu nadciśnienia tętniczego u chorego z GFR <30 ml/min/1,73 m2:""",
...
Question(
            prompt="""8. Wskaż lek stosowany w leczeniu nadciśnienia tętniczego, który może spowodować ostry napad dny moczanowej:""",
...
Question(
            prompt="""9. Wskaż lek stosowany w leczeniu nadciśnienia tętniczego, który może spowodować suchy kaszel:""",
...
Question(
            prompt="""10. Wskaż lek stosowany w leczeniu nadciśnienia tętniczego, który może spowodować obrzęk okolicy kostek:""",
...
@app.route('/')
def index():
    session['score'] = 0  # Inicjalizuj wynik w sesji
    session['current_question'] = 0  # Zaczynaj od pierwszego pytania
...
def question():
    ...
    if request.method == 'POST':
        ...
        return render_template('question.html', question=question, correct=correct, selected_option=selected_option, show_explanation=True)

    return render_template('question.html', question=question, show_explanation=False)

@app.route('/summary')
def summary():
    score = session.get('score', 0)
    return render_template('summary.html', score=score, total=len(questions))
